...
compare_d <- read.csv("Comparison_Synteny_Diopt_Data_LOC_Final.csv",header = TRUE)
...
if(data == "XT"){  
  ...
  total_data <- data.frame(data = c(unique_XT_syn_d$XT_synteny,unique_XT_diopt_d$XT_Diopt)) %>% distinct()
  ...
  if(nrow(a) > 3){
    ...
    filtered_inter_d <- filtered_inter_d %>% arrange(desc(total_present_count))
    
    if(nrow(filtered_inter_d) > 3 ){
    
      filtered_complete_data <- filtered_inter_d[1:3,] %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
    
     #filtered_complete_data <- filtered_inter_data %>% distinct()  
    
    }
    
    else if(nrow(filtered_inter_d) <= 3){
      
      filtered_complete_data <- filtered_inter_d %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny
      )
    ...
    filtered_data_o <- data.frame()
    
    if( unique(a$XT_HS_Presence_Synteny) %in% FALSE & unique(a$XT_HS_Presence) %in% FALSE ){
      
      filtered_data_other[[length(filtered_data_other)+1]] <- a %>% filter(XT_HS_Presence == FALSE & XT_HS_Presence_Synteny == FALSE) %>% distinct()    
      filtered_data_o <- do.call("rbind.data.frame",filtered_data_other)
      filtered_data_o <- filtered_data_o %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
      
    }
    
    #filtered_complete_df <- filtered_complete_data[1:3,]
    
    if(nrow(filtered_complete_data) > 3){
      
      filtered_complete_data <- filtered_complete_data[1:3,] %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
      
    }
    
    if(nrow(filtered_data_o) > 0){
      
      filtered_complete_d <- rbind.data.frame(filtered_data_o,filtered_complete_data) %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
      
    }
    
    else if(nrow(filtered_data_o) == 0){
      
      filtered_complete_d <- filtered_complete_data %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
      
    }

    xb_grouped_list[[length(xb_grouped_list) + 1]] <- filtered_complete_d 
   ...
   xb_grouped_list[[length(xb_grouped_list) + 1]] <- a %>% select(XT_synteny,HS_synteny,XL_L,XL_S,MS,DR,XT_Gene_ID_Synteny,XB_ID,HS_Gene_ID_Synteny,XL_L_Gene_ID,XL_L_ID,XL_S_Gene_ID,XL_S_ID,MS_Gene_ID,DR_Gene_ID,XT_Diopt,HS_Diopt,ortho,swiftortho,phylome,proteinortho,Inparanoid,Sonic,FastOrtho,XT_Gene_ID_Diopt,HS_Gene_ID_Diopt,XT_HS_Presence,weightage_score_diopt,weightage_score_syn,XT_HS_Presence_Synteny)
  ...
  #write.csv(filtered_entire_data,"Comparison_Synteny_Diopt_D_Final_LOC_XT.csv",row.names = F,col.names = T,quote = F)
  
  write_xlsx(filtered_entire_data,path = "Comparison_Synteny_Diopt_D_Final_LOC_XT.xlsx",col_names = TRUE)
